...
import {
  ...
  Target, Landmark, Activity, Zap, Brain,
...
function CardHeader({ children, accent }: { children: React.ReactNode; accent?: string }) {
  return (
...
// ── TAB: AI Analyzer ──────────────────────────────────────────────────────────
function AITab({ orgCode, googleConnected, microsoftConnected }: {
  orgCode: string; googleConnected: boolean; microsoftConnected: boolean;
}) {
    ...
    <div className="space-y-5">
      ...
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b flex items-center gap-3"
          ...
          <div>
            <h2 className="text-[14px] font-bold text-slate-100">AI Financial Model Builder</h2>
            <p className="text-[11px] text-slate-500">
              Connect your cloud storage → select any spreadsheet or doc → Fundir AI builds the full model with projections
            </p>
          </div>
          {anyConnected && (
        ...
        <div className="px-5 pt-4 pb-2">
          <div className="flex items-center gap-3 mb-5">
            {['Connect cloud storage', 'Select financial file', 'AI builds model'].map((step, i) => (
  ...
  return (
    <div style={{ background: '#070d1a', minHeight: '100vh' }}>
      ...
      <div className="px-8 py-7 max-w-7xl mx-auto">
        ...
        {tab === 'sites'    && <SitesTab />}
        {tab === 'ai'       && <AITab orgCode={orgCode} googleConnected={googleConnected} microsoftConnected={microsoftConnected} />}
      </div>
