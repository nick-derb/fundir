    ...
    <div className="space-y-5">
      <Card className="overflow-hidden">
        <div className="px-5 py-4 border-b flex items-center gap-3"
          style={{ borderColor: 'var(--border-hairline)', background: 'var(--bg-elevated)' }}>
          <div className="w-7 h-7 rounded-[6px] flex items-center justify-center flex-shrink-0"
            style={{ background: 'var(--accent-tint)' }}>
            <Brain className="w-3.5 h-3.5 text-accent" />
          </div>
          <div>
            <h2 className="text-[14px] font-bold text-primary">AI Document Analyzer</h2>
          ...
          {anyConnected && (
            <span className="ml-auto text-[11px] font-bold text-success flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-success/20 flex-shrink-0"
...
export const DOC_BADGE: Record<string, { bg: string; text: string; label: string }> = {
  financial:         { bg: 'var(--accent-tint)',  text: 'var(--accent)', label: 'Financial'      },
  strategic_plan:    { bg: 'var(--info-tint)',  text: 'var(--info)', label: 'Strategic Plan' },
  grant_application: { bg: 'var(--info-tint)',  text: 'var(--info)', label: 'Grant App'      },
  program_report:    { bg: 'var(--success-tint)',   text: 'var(--success)', label: 'Program Report' },
  board_minutes:     { bg: 'var(--warning-tint)',  text: 'var(--warning)', label: 'Board Minutes'  },
  general:           { bg: 'var(--bg-elevated)', text: 'var(--text-secondary)', label: 'General'        },
};
  ...
  if (docType === 'financial') {
    ...
    return (
      <div className="space-y-3">
        <p className="text-[12px] text-secondary leading-relaxed">{summary}</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[
            { label: 'Total Revenue',   value: rev >= 1_000_000 ? `$${(rev/1_000_000).toFixed(2)}M` : `$${(rev/1000).toFixed(0)}K`,       color: 'var(--accent)' },
            { label: 'Net Income',      value: `${net >= 0 ? '+' : ''}$${Math.abs(net/1000).toFixed(0)}K`,  color: net >= 0 ? 'var(--success)' : 'var(--critical)' },
            { label: 'Net Assets',      value: assets >= 1_000_000 ? `$${(assets/1_000_000).toFixed(2)}M` : `$${(assets/1000).toFixed(0)}K`, color: 'var(--info)' },
            { label: 'Grant Readiness', value: `${score}/100`,                                                color: score >= 70 ? 'var(--success)' : 'var(--warning)' },
          ].map(({ label, value, color }) => (
            <div key={label} className="p-3 rounded-[8px] border"
              style={{ background: 'var(--badge-bg)', borderColor: 'var(--card-border)' }}>
              <p className="text-[9px] text-tertiary uppercase tracking-wide mb-1">{label}</p>
              <p className="text-[15px] font-bold font-mono" style={{ color }}>{value}</p>
            </div>
          ))}
        </div>
        {months != null && (
          <p className="text-[11px] text-tertiary">{months} months of cash reserves</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {sections.slice(0, 2).map((s, i) => (
        <div key={i}>
          <p className="text-[10px] font-bold text-tertiary uppercase tracking-widest mb-1.5">{s.title}</p>
          <div className="space-y-1">
            {s.items.slice(0, 3).map((item, j) => (
              <div key={j} className="flex items-start gap-2">
                <span className="w-1 h-1 rounded-full mt-1.5 flex-shrink-0 bg-teal-500" />
                <p className="text-[11px] text-secondary">
                  <strong className="text-muted">{item.label}:</strong> {item.value}
                </p>
              </div>
            ))}
          </div>
      ...
      {grantAlignment.length > 0 && (
        <div>
          <p className="text-[10px] font-bold text-tertiary uppercase tracking-widest mb-1.5">Grant Alignment</p>
          {grantAlignment.slice(0, 2).map((g, i) => {
            const c = g.potential === 'high' ? { bg: 'var(--success-tint)', text: 'var(--success)' } : { bg: 'var(--warning-tint)', text: 'var(--warning)' };
            return (
              <div key={i} className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[11px] text-secondary font-semibold">{g.area}</p>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[15px] font-bold text-primary">Document Library</h2>
          <p className="text-[11px] text-secondary mt-0.5">
            AI-analyzed documents for {orgName ?? orgCode} — click any row to expand
          </p>
        </div>
        <button onClick={onSwitchToAnalyzer}
          className="flex items-center gap-2 px-4 py-2 rounded-[8px] text-[12px] font-bold text-primary"
      ...
      {!loading && docs.length === 0 && (
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <div>
            <p className="text-[14px] font-bold text-secondary">No documents analyzed yet</p>
          ...
          <button onClick={onSwitchToAnalyzer}
            className="flex items-center gap-2 px-5 py-2.5 rounded-[8px] text-[12px] font-bold text-primary"
            style={{ background: 'var(--accent)' }}>
              ...
              <div key={doc.id} className="rounded-[10px] border overflow-hidden transition-all"
                ...
                <button onClick={() => handleExpand(doc.id)}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[13px] font-semibold text-primary truncate">{doc.file_name}</span>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className="text-[11px] text-tertiary">{timeAgo(doc.analyzed_at)}</span>
                    <ChevronDown className="w-4 h-4 text-tertiary transition-transform"
                ...
                {isOpen && (
                  <div className="px-5 pb-5 border-t" style={{ borderColor: 'var(--card-border)' }}>
                    {fetching ? (
                      <div className="flex items-center gap-2 py-4 text-tertiary text-[12px]">
...
function readinessBadge(r: 'high' | 'medium' | 'low') {
  return r === 'high' ? 'var(--success)' : r === 'medium' ? 'var(--warning)' : 'var(--critical)';
}
function urgencyBadge(u: 'high' | 'medium' | 'low') {
  return u === 'high' ? 'var(--critical)' : u === 'medium' ? 'var(--warning)' : 'var(--text-secondary)';
}
    ...
    return (
      <div className="flex flex-col items-center gap-5 py-16">
        ...
        <div className="text-center">
          <p className="text-[14px] font-bold text-primary">Synthesizing strategy brief…</p>
          <p className="text-[12px] text-secondary mt-1">Analyzing your documents and building your funding roadmap</p>
          <div className="flex items-center justify-center gap-3 mt-3">
            {['Reading documents', 'Identifying patterns', 'Building strategy'].map((s, i) => (
              <span key={s} className="flex items-center gap-1 text-[10px] text-tertiary">
  ...
  if (!brief) {
    return (
      <div className="flex flex-col items-center gap-6 py-16 text-center">
        ...
        <div>
          <h2 className="text-[17px] font-bold text-primary">AI Strategy Brief</h2>
          <p className="text-[12px] text-secondary mt-2 max-w-md">
        ...
        {error && (
          <div className="flex items-center gap-2 px-4 py-3 rounded-[8px] border max-w-md"
            style={{ background: 'var(--critical-tint)', borderColor: 'var(--critical-tint)' }}>
        ...
        <p className="text-[11px] text-tertiary">Analyzes all documents in your library · ~30 seconds</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-[10px] font-bold text-info uppercase tracking-widest mb-0.5">AI · Strategy Brief</p>
          <h2 className="text-[20px] font-bold text-primary">{brief.org_name} Funding Strategy</h2>
          <p className="text-[11px] text-secondary">
        ...
        <button onClick={() => setBrief(null)}
          className="flex items-center gap-1 text-[11px] text-tertiary hover:text-info transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-5">
          <p className="text-[11px] font-bold text-success uppercase tracking-widest mb-3">Organizational Strengths</p>
          <div className="space-y-2">
        ...
        <Card className="p-5">
          <p className="text-[11px] font-bold text-critical uppercase tracking-widest mb-3">Critical Gaps</p>
          <div className="space-y-2.5">
            {brief.critical_gaps.map((g, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[12px] text-muted font-semibold">{g.gap}</p>
                  <p className="text-[11px] text-tertiary mt-0.5">{g.impact}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-[13px] font-bold text-primary">Funding Strategy</p>
        </div>
        <p className="text-[12px] text-secondary leading-relaxed whitespace-pre-line">{brief.funding_strategy}</p>
      </Card>
          ...
          {brief.top_funding_priorities.map(p => (
            <Card key={p.rank} className="p-4">
              <div className="flex items-start gap-4">
                <span className="text-[20px] font-bold text-tertiary w-6 flex-shrink-0">#{p.rank}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-[13px] font-bold text-primary">{p.focus_area}</p>
                  ...
                  <p className="text-[12px] text-secondary mb-2">{p.rationale}</p>
                  <div className="flex items-center gap-4 text-[11px] text-tertiary">
                    {p.target_amount && <span>Target: <strong className="text-muted">{p.target_amount}</strong></span>}
                    <span>Timeline: <strong className="text-muted">{p.timeline}</strong></span>
                  </div>
                </div>
              </div>
            </Card>
          ...
          {brief.grant_opportunities.map((g, i) => (
            <Card key={i} className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div>
                  <p className="text-[13px] font-bold text-primary">{g.focus_area}</p>
                  <span className="text-[10px] font-bold text-secondary uppercase">{g.type}</span>
                </div>
              ...
              <p className="text-[11px] text-secondary mb-2">{g.rationale}</p>
              {g.estimated_range && <p className="text-[11px] font-bold text-accent">{g.estimated_range}</p>}
            </Card>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {brief.quarterly_action_plan.map(q => (
            <Card key={q.quarter} className="p-4">
              <p className="text-[11px] font-bold text-info uppercase tracking-widest mb-3">{q.quarter}</p>
              <div className="space-y-2">
                {q.actions.map((action, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <ArrowRight className="w-3 h-3 text-tertiary flex-shrink-0 mt-0.5" />
                    <p className="text-[11px] text-secondary leading-snug">{action}</p>
                  </div>
                ))}
              </div>
            </Card>
      ...
      <Card className="p-5">
        <p className="text-[11px] font-bold text-accent uppercase tracking-widest mb-2">Revenue Diversification</p>
        <p className="text-[12px] text-secondary leading-relaxed">{brief.revenue_diversification}</p>
      </Card>

      {brief.risk_mitigation.length > 0 && (
        <Card className="p-5">
          <p className="text-[11px] font-bold text-warning uppercase tracking-widest mb-3">Risk Mitigation</p>
          <div className="space-y-2">
            {brief.risk_mitigation.map((r, i) => (
              <div key={i} className="flex items-start gap-2">
                <AlertTriangle className="w-3.5 h-3.5 text-warning flex-shrink-0 mt-0.5" />
                <p className="text-[12px] text-secondary">{r}</p>
              </div>
            ))}
          </div>
        </Card>
...
export function OrgFinancialsTab({
  ...
  if (!financialData?.computed) {
    return (
      <div className="flex flex-col items-center gap-5 py-20 text-center">
        ...
        <Link href="/settings"
          className="flex items-center gap-2 px-5 py-2.5 rounded-[8px] text-[13px] font-bold text-primary"
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-widest text-accent mb-0.5">IRS Form 990 · ProPublica</p>
        ...
        {[
          {
            ...
            icon: DollarSign, color: 'var(--accent)', sub: `FY${computed.filingYear}`,
          },
          {
        ...
        <div className="rounded-xl border p-5" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
          ...
          <div className="space-y-4">
            {[
              ...
              { label: 'Private Grants & Gifts', pct: computed.privateGrantsPct, color: 'var(--accent)' },
      ...
      <div className="relative overflow-hidden border-b" style={{
        ...
        <div className="relative px-8 py-7 max-w-7xl mx-auto">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold text-accent uppercase tracking-widest">Org Intelligence · Financials</span>
          </div>
